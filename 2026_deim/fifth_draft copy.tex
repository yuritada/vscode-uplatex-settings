...
既存研究には，LLMによる3D生成・編集や，視覚フィードバックを用いた反復的改善の枠組みが提案されている．一方で，(a) 数値制約に基づく体系的な検査（quality checking），(b) 検査結果に基づく自動修正の反復（self-correction loop），(c) セッションを跨いで知識を永続化し効率を改善する仕組み（knowledge inheritance）を，統合的に扱う研究は依然として限定的である．本研究はこの統合に焦点を当て，幾何・法規に由来する数値制約を扱う品質保証ループと，二層知識ベースによる継承メカニズムを提案する．
