...
- **生成:** プランナーの指示に基づき，ツール群がBlender APIを介して3Dシーンを操作する．
- **検査:** 操作直後，検査器（T3）がStaticDBに基づきシーン全体をスキャンする．本検査プロセスにおいてはVLMによる曖昧な視覚判定ではなく，バウンディングボックスや頂点座標に基づいた厳密な数値検査が行われる．
- **修正:** 違反が検出された場合，検査器は「ドアが床から0.5m浮いている」といった具体的な数値情報を含むエラーメッセージをAIコアにフィードバックする．LLMはエラーメッセージに含まれる数値情報に基づき，「Z軸方向に-0.5m移動させる」といった正確な修正アクションを立案・実行する．
...
- **ドアの浮遊**: ドア下端が地面（Z=0）から0.5m浮いている（物理的整合性の欠如）．
- **ドアの壁高超過**: ドアの上端が壁のバウンディングボックスを逸脱している（包含関係の違反）．
- **窓の壁高超過**: 窓の上端が壁の高さを超えている（幾何学的違反）．
- **窓とドアの重複**: 窓とドアの配置位置が物理的に干渉している（衝突）．
- **採光面積不足**: 窓の面積が床面積の1/7未満である（建築基準法に基づく法規違反）．
...
- **初期段階**: エージェントはまず窓を配置し，検証ツールで\texttt{Overlap Detected}というエラーを受け取ってから，少しずつ位置をずらす対処療法的な修正を行った．
